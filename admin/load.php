<?php
require_once __DIR__.'/../Connection/Connection.php';
require_once __DIR__.'/Controller/AbstractController.php';
require_once __DIR__ . '/Controller/HomepageController.php';
require_once __DIR__.'/Controller/AdminLoginCheck.php';
require_once __DIR__ .'/Controller/ArticleController.php';
require_once __DIR__ . '/Controller/CategoriesController.php';
require_once __DIR__ . '/Controller/CommentController.php';
require_once __DIR__ . '/Controller/AdminController.php';
require_once __DIR__ . '/Controller/UserController.php';
require_once __DIR__ . '/../src/entity/Admin.php';
require_once __DIR__ . '/../src/entity/Article.php';
require_once __DIR__ . '/../src/entity/ArticleCategories.php';
require_once __DIR__ . '/../src/entity/Tags.php';
require_once __DIR__ . '/../src/entity/Categories.php';
require_once __DIR__ . '/../src/entity/Comments.php';
require_once __DIR__ . '/../src/entity/User.php';
require_once __DIR__ . '/../src/repository/ArticleRepository.php';
require_once __DIR__ . '/../src/repository/CommentRepository.php';
require_once __DIR__ . '/../src/repository/ArticleCategoriesRepository.php';
require_once __DIR__ . '/../src/repository/TagsRepository.php';
require_once __DIR__ . '/../src/repository/CategoriesRepository.php';
require_once __DIR__ . '/../src/repository/UserRepository.php';
require_once __DIR__ . '/../src/repository/AdminRepository.php';

